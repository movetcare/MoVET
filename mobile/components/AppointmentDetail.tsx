   ...
   useEffect(() => {
     if (didConfirm && appointment) {
       ...
       const updateAppointmentStatus = async () => {
         const updateAppointment = httpsCallable(
           functions,
           "updateAppointment",
         );
     ...
     <Screen>
       ...
       >
         ...
         {appointment?.notes?.includes("Appointment Location:") ? (
           ...
           <>
             ...
             <ItalicText
               ...
               noDarkMode
             >
               UNKNOWN - Please contact us ASAP with the correct address for
               your housecall appointment.
             </ItalicText>
             {appointment?.start?.toDate() >= new Date() ? (
               <ActionButton
         ...
         {appointment?.patients?.map((patient: Patient, index: number) => {
           const [month, day, year] = (patient?.birthday as any).split("-");
         ...
         {appointment?.locationType === "Virtually" &&
           ...
           appointment?.status !== "COMPLETE" && (
             <ItalicText style={tw`text-sm text-center mt-4`}>
               * Virtual Consultations are performed via a secure third party
               web application. A modern web browser with camera and microphone
           ...
           {isLoading ? (
             ...
             <>
               ...
               <TextInput
                 ...
                 control={control}
                 error={
                   (errors["cancellationReason"] as any)?.message as string
                 }

...
const DEBUG = true;
export const handleOpeningConfigurationUpdate = functions.firestore
  .document("configuration/openings")
  .onWrite(async (change: any) => {
    if (change.after.data() !== undefined) {
      const data = change.after.data();
      if (DEBUG) console.log("handleOpeningConfigurationUpdate => DATA", data);
          ...
          message: [
            {
              type: "section",
              text: {
                text: ":door: _Opening Hours Updated!_",
                type: "mrkdwn",
              },
              fields: [
                {

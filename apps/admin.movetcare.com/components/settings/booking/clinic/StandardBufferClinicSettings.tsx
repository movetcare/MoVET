...
const StandardBufferClinicSettings = () => {
    ...
    useEffect(() => {
        const unsubscribe = onSnapshot(
            ...
            (doc: any) => {
                if (doc.data()?.clinicAppointmentBufferTime)
    ...
    const saveChanges = async () => {
        if (didTouchBufferTime && selectedBufferTime && selectedBufferTime !== "")
            await setDoc(
                ...
                .then(() =>
                    toast(`Clinic Appointment Buffer Time Update to "${selectedBufferTime}"`, {
                        position: "top-center",
                        icon: (
                            <FontAwesomeIcon
                                icon={faCheckCircle}
                ...
                .catch((error: any) =>
                    toast(`Clinic Appointment Buffer Time Update FAILED: ${error?.message}`, {
                        duration: 5000,
    ...
    return error ? (
        ...
        <li className="py-4 flex-col sm:flex-row items-center justify-center">
            <div className="flex flex-col mr-4">
                ...
                <p className="text-sm">
                    This controls how many minutes of buffer time is added between all appointments.
                </p>
            </div>
...
export default StandardBufferClinicSettings;
